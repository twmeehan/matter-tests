...
for name in names:
    ...
    info = np.loadtxt("dumps/" + basename + name + "/info.txt")
    Np = np.loadtxt("dumps/" + basename + name + "/out_part_frame_0.csv", delimiter=",", skiprows=1).shape[0]
    ...
    for f in frames:
        pq_data = np.loadtxt("dumps/" + basename + name + "/out_pq_frame_" + str(f) + ".csv", delimiter=",")
        mean_p[f] = pq_data[0]
        mean_q[f] = pq_data[1]
...
for n in range(0, len(names)):
    plt.plot(-eps_ax_list[n], mean_p_list[n]/1e6, '.-',  label = names[n].replace("_", " "))
...
plt.subplot(212)
for n in range(0, len(names)):
    plt.plot(-eps_ax_list[n], mean_q_list[n]/1e6, '.-', label = names[n].replace("_", " "))
