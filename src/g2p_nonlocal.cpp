...
void Simulation::G2P_nonlocal(){
    ...
    int n_support = std::ceil(nonlocal_l / dx);
    ...
    #pragma omp parallel num_threads(n_threads)
    {
        ...
        for(int p = 0; p < Np; p++){
            ...
            T delta_gamma_p = 0;
            T divisor_p = 0;
            unsigned int i_base = std::floor((xp(0)-grid.xc)*one_over_dx) - (n_support-1); // the subtraction of one is valid for both quadratic and cubic splines
            unsigned int j_base = std::floor((xp(1)-grid.yc)*one_over_dx) - (n_support-1);
            unsigned int k_base = std::floor((xp(2)-grid.zc)*one_over_dx) - (n_support-1);

            for(int i = i_base; i < i_base+(2*n_support); i++){
                T xi = grid.x[i];
                for(int j = j_base; j < j_base+(2*n_support); j++){
                    T yi = grid.y[j];
                    for(int k = k_base; k < k_base+(2*n_support); k++){
                        T zi = grid.z[k];
        ...
        {
            for(int p = 0; p < Np; p++){
                particles.delta_gamma[p] += particles_delta_gamma_local[p];
    ...
    for (int p = 0; p<Np; p++){
        ...
        if (divisor_p > 0)
            particles.delta_gamma[p] /= divisor_p;
        else
